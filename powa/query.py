...
class QueryOverviewMetricGroup(MetricGroupDef):
    ...
    def query(self):
        ...
        cols = [to_epoch(c.ts),
                ...
                sum(c.blk_read_time).label("blk_read_time"),
                sum(c.blk_write_time).label("blk_write_time"),
        ...
        if self.has_extension("pg_stat_kcache"):
            ...
            total_time_percent = lambda x: least(100, (x * 100) /
                                                 total_time)
            cols.extend([
                sum(kc.reads).label("reads"),
                sum(kc.writes).label("writes"),
